 ...
 *
 */
var FuncInit= FuncInit || {};var FuncInit={idtree:'#jqxTree',idplyr:'#player',videoid:'',plyLstId:'PLrwe0-1smC6zGKsy0ggtEJWAQDaBC2n4i',plys:null,h:0,w:0,fact:1
...
{id:'electro',label:'electro',value:'PLrwe0-1smC6w8At_JO2HcJdpucIqn6im1'},{id:'sphh',label:'special hip-hop',value:'PLrwe0-1smC6xH0EtS7-2W2K422EVR19bk'},{id:'sprg',label:'special reggae',value:'PLrwe0-1smC6xvoITpy0zzwirUm2D_4n3b'}],tmp:'',PlrR:function(event){player.loadPlaylist({list:FuncInit.plyLstId,listType:'playlist',index:0,startSeconds:1,suggestedQuality:'default'});event.target.pauseVideo();var arr= FuncInit.plys.getPlaylist();alert(arr);FuncTree.add(arr);},PlrStd:function(event){player.setVolume(0);event.target.pauseVideo();},rchg:function(){FuncInit.plys.destroy();onYouTubeIframeAPIReady();},init:function(){var tag  =document.createElement('script');tag.src = "https://www.youtube.com/iframe_api";var firstScriptTag = document.getElementsByTagName('script')[0];firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);
$('#jqxTree').mouseleave(function(){if(FuncInit.plys!=null){FuncInit.plys.setSize(FuncInit.w*0.9,FuncInit.h);};$('#jqxTree').animate({'width':FuncInit.w*0.05});});
$('#jqxTree').mouseenter(function(){if(FuncInit.plys!=null){FuncInit.plys.setSize(FuncInit.w*0.67,FuncInit.h);};$('#jqxTree').animate({'width':(FuncInit.w*0.3)});});
}};
FuncInit.init();FuncInit.h =$(window).height();FuncInit.w=$(window).width();
$(window).resize(function () { FuncInit.h =$(window).height();FuncInit.w=$(window).width();$('#jqxTree').animate({'height':(FuncInit.h)});$('#jqxTree').animate({'width':(FuncInit.w*0.3)});if(FuncInit.plys!=null){FuncInit.plys.setSize(FuncInit.w*67/100,FuncInit.h);}}).resize();
var FuncTree=FuncTree || {};var FuncTree={
bchk:false,bgrow:false,bms:true,updBackG:"body",ptbid:['PLrwe0-1smC6xL233WdVGDdaCugFfd5cbW'],
add:function(arr){$('#jqxTree').jqxTree('collapseAll');var it = $('#jqxTree').jqxTree('getSelectedItem');var elmt = it.element;var lg=arr.length;var elmt=$('#jqxTree').jqxTree('getSelectedItem');var felmt=[];for(var i=lg-1;i>=0;i--){var uri='<iframe id="'+arr[i]+'" sandbox="Access-Control-Allow-Origin allow-scripts" type="text/html" width="250" height="50"src="http://www.youtube.com/embed/'+arr[i]+'?autoplay=0&origin=http://runsense.re"frameborder="0" enablejsapi="1" />';var rplc='#'+arr[i];$('#jqxTree').jqxTree('addTo', {id:arr[i], label:'play '+i,value:arr[i],items:[{html:uri, disabled:true}] },elmt); var itm=$('#jqxTree').jqxTree('getItem', $(rplc)[0]);felmt[i]=itm.element;}$('#jqxTree').jqxTree('expandItem', elmt);$('#jqxTree').jqxTree('expandItem', felmt[felmt.length-1]);/*for(var i in felmt)  $('#jqxTree').jqxTree('expandItem', felmt[i]);$('#jqxTree').jqxTree('refresh');*/},
init:function(){var w=FuncInit.w*30/100;$(FuncInit.idtree).jqxTree({source:FuncInit.src,width:w,height:FuncInit.h});
