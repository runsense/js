//<![CDATA[
        ...
        $(function() {
		MapsLib.initialize();
		$("pano").bind('click',function(){ $(this).animate({ width: '35%' });$(FuncInit.idmap).animate({ width: '35%' });});
		...
		$(FuncInit.idtab).on("mouseenter",function(){if(FuncInit.bnm){ $(this).animate({opacity: '0.75',height: '100%',width: '70%'});$(FuncInit.idmap).animate({opacity: '0.3'});$(FuncInit.idp).animate({opacity: '0.3'});$('body').animate({height: '100% !important',margin: '0px !important',padding: '0px !important'});}$(FuncInit.idinf).empty();FuncTree.append("<small>Revenir MAP </small>(A DROITE)","green");$(FuncInit.idinf).bind('click',function(){ if(FuncInit.bnm){$(FuncInit.idtab).animate({opacity: '1',height: '15%',width: '100%'});$(FuncInit.idmap).animate({opacity: '1'});$(FuncInit.idp).animate({opacity: '1'});}});});
		...
		$("#r_bdd").change(function(){FuncTree.bms=false;FuncTree.bms=false;MapsLib.srchOnAll(this.value);$(FuncInit.idinf).empty();FuncTab.msg= 'Pour Afficher les donners, cliquer ailleurs!!';FuncTree.append(FuncTab.msg,'red');});
		$("#r_bdd").mouseleave(function(){FuncTab.crTb();MapsLib.displayList();FuncTab.fshBDD();FuncTab.search='';});
		$("#r_bdd").click(function(){$(FuncInit.idinf).empty();FuncTab.msg= 'Pour Afficher les donners, visoinner les donner en bas de l Ecran!!';FuncTree.append(FuncTab.msg,'blue');});
		$(FuncInit.idbtn).bind('click',function(){$("#pano").empty();$("#pano").animate({ width: '0%' });$(FuncInit.idp).animate({left: '70%'});$('small').show();$(FuncInit.idinf).hide();$('#dep').val(null);$('#arv').val(null);$(FuncInit.idtab).show();$(FuncInit.iditi).css('color','blue');$(FuncInit.iditi).css('border-color','#FFFF00');$(FuncInit.iditi).css('font-size','15px');$(FuncInit.iditi).css('font-weight','none');MapsLib.s=null;MapsLib.e=null;$(FuncInit.idbtn).hide();MapsLib.initialize();$(FuncInit.idtab).show();});
		...
		$('#dep').click(function(){MapsLib.additi('dep'); return true;});
		$('#arv').click(function(){MapsLib.additi('arv'); return true;});
