...
displayList: function(json) {
    ...
    else { FuncTab.cRows(rows) }
},
cRows: function(rows)
   {
	var l = rows.length;var list_table = "<table class='table' id ='list_table'><tbody>";
	var lat=null;
		var lng=null;
		var row;
      ...
      for ( row in rows) {
        ...
        list_table += "\
          <tr id="+nom.replace(/ /g,'').replace(/'/g,'')+">\
			<td >" +ctg + "</td><td >" + nom + "</td><td >" + desc + "</td><td >" +ctg + "</td><td style='color:blue;width:20px;' >" + lat + "</td><td style='color:blue;width:20px;' >" + lng + "</td>\
          </tr>";
      }
list_table += "</tbody></table>";
		if(lat!=""&&lng!="") {if(lng<55.8){MapsLib.map_centroid= new google.maps.LatLng(lat,lng);map.setCenter(MapsLib.map_centroid);}}
