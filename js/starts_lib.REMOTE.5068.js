...
I:{x:'#jqxTree',t:'#listv',m:'#map_canvas',p:'#panel',i:'#info',r:'#iti',b:'#btn',o:'#pano'},
...
/*init:function(){
	...
	$("#fl").bind('click',function(){M.findMe();});$(F.I.b).bind('click',function(){$(F.I.o).empty();$(F.I.o).animate({width:'0%'});
	$(F.I.p).animate({left: '70%'});$('small').show();$(F.I.i).empty();$('#dep').val(null);$('#arv').val(null);
	$(F.I.t).show();$(F.I.r).css('color','blue');$(F.I.r).css('border-color','#FFFF00');$(F.I.r).css('font-size','15px');
	$(F.I.r).css('font-weight','none');M.s=null;M.e=null;$(F.I.b).hide();M.initialize();$(F.I.t).show();});
	$('#clear').css('color','blue');$('#clear').bind('click',function(){F.b.b=false;
	M.map_centroid=new google.maps.LatLng(0,0);
	H.zoom=5;
	$.each(F.srcZn,function(){var rplc='#'+this.val;$(F.I.x).jqxTree('collapseItem',$(rplc)[0]);});
	$(F.I.x).jqxTree('refresh');M.polygonTableID=['15JWgv5XJiGI7iqzsLQKOakEniw04ydBtvQYx2M0a'];$.ajax({sucess:M.doSearch()});F.b.b=true;});
	$('#find_me').click(function(){M.findMe();return false;});$('#dep').click(function(){M.chad='#dep'; return true;});
	$('#arv').click(function(){M.chad='#arv'; return true;});$(F.I.m).mouseover(function(){});
	$(F.I.m).mouseleave(function(){if(map){if(map.getMapTypeId()===google.maps.MapTypeId.ROADMAP){map.setMapTypeId(google.maps.MapTypeId.HYBRID);}
	...
	$(F.I.r).click(function(){R.calcRoute();return true;});},*/
kpe:function(e){
...
selectBox:function(i){if(i.id.match('_')===null){var d=i.id;F.tmp='';F.tmp=F.srcZn[d]?d:F.tmp;F.tmp==''?$.each(F.srcId,function(){F.tmp=this.label===d?this.id:F.tmp;}):'';F.tmp=F.tmp==''?d.split('_')[0]:F.tmp;F.b.f=true;}else{$("#r_theme").selectBox('value',i.id.split('_')[1]);F.b.f=false;}},init:function(){var w=$(window).width()/3.5;M.initialize();var exp=true;$(F.I.x).jqxTree({source:F.src,width:w+'px',height:F.H/2}),
