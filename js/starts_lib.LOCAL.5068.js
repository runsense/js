...
refcb:function(){$.each(F.src,function(){
	...
	itmRef:function(json){
		...
		try{
		F.src[Number(this[0])].items.push(
		{
			id:nm,
...
if(!isNaN(A.lat)&&!isNaN(A.lng)){M.map_centroid=new google.maps.LatLng(A.lat,A.lng);map.setCenter(M.map_centroid);}
A.results.append(A.list_table);
...
$(F.Is[2]).animate({opacity:'0.7'});b=false;}}).on('mouseover','tr',function(){M.addSrchMarker.setMap(null);var nm=$(this).children('td:nth-child(1)').text();
	...
	tabToMap:function(lat,lng,str,b){
		...
		H.zoom=F.b.c?4:7;
		H.rvzoom=H.zoom==4?7:H.zoom==7?9:4;
		map.setCenter(
			M.map_centroid
		),			
		map.setMapTypeId(
